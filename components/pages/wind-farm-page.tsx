    ...
    <AppShell activePath="/wind-farms">
      ...
      <section className="farm-summary-bar">
        ...
        <div>
          <small>平均健康度</small>
          <strong>
            {windFarm.averageHealthScore.toFixed(1)} <span>%</span>
          </strong>
        </div>
        <div>
          <small>活跃告警</small>
          <strong className="critical-text">{windFarm.activeAlarmCount}</strong>
        </div>
        <div>
          <small>AI Missions</small>
          <strong className="info-text">{windFarm.activeMissionCount}</strong>
        </div>
      </section>
      ...
      <section className="data-toolbar">
        ...
        <span className="toolbar-result">
          显示 {filtered.length} / {turbines.length} 台机组
        </span>
        ...
        <Card className="data-table-card">
          <div className="table-scroll">
            <table className="data-table">
              ...
              <tbody>
                {filtered.map((turbine) => (
                  <tr key={turbine.id} onClick={() => setSelected(turbine)}>
                    <td>
                    ...
                    <td>
                      <strong>{turbine.powerMW.toFixed(2)} MW</strong>
                    </td>
                    <td>{turbine.windSpeedMps.toFixed(1)} m/s</td>
                    <td>{turbine.rotorSpeedRpm.toFixed(1)} rpm</td>
                    <td>
                      <HealthBadge score={turbine.healthScore} />
                    </td>
                    <td>{turbine.activeAlarmCount}</td>
                    <td className="mono">{turbine.currentMissionId ?? "—"}</td>
                    <td>{new Date(turbine.lastMaintenanceAt).toLocaleDateString("zh-CN")}</td>
                    <td>
