...
function AlarmDrawer({ alarm, onClose }: { alarm: Alarm; onClose: () => void }) {
  ...
  const featured = alarm.id === "ALARM-0031";
  return (
    <>
      ...
      >
        ...
        <section className="drawer-section">
          <div className="drawer-section__title">
            <h3>证据链</h3>
            <span className="record-count">{relatedEvidence.length} 项</span>
          </div>
          <div className="evidence-compact-list">
            {relatedEvidence.length ? (
              ...
              <p className="no-evidence">AI 证据正在整理中</p>
            )}
          </div>
        </section>
        <section className="drawer-section">
          ...
          <div className="alarm-audit">
            ...
            <div>
              ...
              <p>
                ...
                <small>Failure Diagnosis Agent · 09:31</small>
              </p>
            </div>
            <div>
              <span>
        ...
        <footer className="detail-drawer__footer">
          ...
          <Button variant="secondary">
            <UserRound size={14} /> 指派
          </Button>
          <a href={`/missions/${featuredMission.id}`} className="button button--primary button--md">
...
export function AlarmCenterPage() {
  ...
  const counts = {
    ...
    major: alarms.filter((item) => item.severity === "major").length,
    warning: alarms.filter((item) => item.severity === "warning").length,
    diagnosed: alarms.filter(
  ...
  return (
    <AppShell activePath="/alarms">
        ...
        meta={
          <>
            <StatusBadge
              ...
              pulse
            />
            <span className="page-meta-text">17 个活跃告警 · 6 个正在 AI 分析</span>
          </>
        }
        actions={
          <>
      ...
      <section className="data-toolbar">
        ...
        <Button variant="secondary">
          <Filter size={14} /> 状态 <ChevronDown size={12} />
        </Button>
        <Button variant="secondary">
          <Clock3 size={14} /> 触发时间 <ChevronDown size={12} />
        </Button>
        ...
        <div className="table-scroll">
          <table className="data-table alarm-table">
            <thead>
              <tr>
                <th>
                  <input type="checkbox" aria-label="选择所有告警" />
                </th>
                <th>Severity</th>
                <th>Wind Turbine</th>
                <th>Subsystem</th>
              ...
              {filtered.map((alarm) => (
                ...
                >
                  <td onClick={(event) => event.stopPropagation()}>
                    <input type="checkbox" aria-label={`选择 ${alarm.id}`} />
                  </td>
                  <td>
                    <StatusBadge
                      value={alarm.severity}
