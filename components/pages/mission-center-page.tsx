  ...
  return (
    ...
    >
      ...
      <div className="kanban-card__footer">
        <span>
          <Clock3 size={11} />{" "}
          {new Date(mission.updatedAt).toLocaleTimeString("zh-CN", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false,
          })}
    ...
    () =>
      displayMissions.filter(
        (mission) =>
          ...
          `${mission.id} ${mission.title} ${mission.turbineId}`
            .toLowerCase()
            .includes(query.toLowerCase()),
      ),
    [displayMissions, query, scope],
  );
    ...
    <AppShell activePath="/missions">
      <PageHeader
        ...
        meta={
          <>
            <StatusBadge
              value="working"
              label={`${displayMissions.length - completedCount} ACTIVE MISSIONS`}
              tone="info"
              pulse
            />
      ...
      <section className="mission-summary">
        <div>
          ...
          <span>
            <small>ACTIVE</small>
            <strong>{displayMissions.length - completedCount}</strong>
          </span>
        </div>
        <div>
      ...
      <section className="data-toolbar mission-toolbar">
        ...
        <Button variant="secondary">
          <Filter size={14} /> Severity <ChevronDown size={12} />
        </Button>
        <Button variant="secondary">
          <Bot size={14} /> Lead Agent <ChevronDown size={12} />
        </Button>
        <span className="toolbar-result">{filtered.length} Missions</span>
        <div className="view-switcher">
          <button className="active" aria-label="看板视图">
            <LayoutGrid size={15} />
          </button>
