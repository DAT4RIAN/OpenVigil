...
`/login` 是独立公开入口，不属于原有 22 个业务页面。文案统一使用“风电智能运维平台”；桌面采用生成的草地、山脊风场图片与操作区双栏，手机优先展示登录操作；沿用雾灰/青绿色 token，支持深浅主题。生产模式连接现有 ChatGPT 托管身份登录，保留安全的业务返回路径；演示模式明确标识演示入口。不得通过公开登录页授予生产能力，不引入虚假的账号密码校验。交互必须覆盖断网提示、跳转等待、重复点击和访问帮助。

图片来源、实际生成提示词及验证边界见 [登录页设计记录](docs/design/login-page.md)。
...
| ----------- | --------- | --------- | -------------------------- |
| Background  | `#edf2f3` | `#0f1b20` | 页面画布                   |
| Surface     | `#fcfefe` | `#17282e` | 卡片 / 表格                |
...
- 主按钮使用 `--on-primary`：浅色主题为 `#f5fcfc`，深色主题为 `#0d252a`；危险与健康实心标识分别使用 `--on-critical`、`--on-success`，保证亮色背景上的文字对比度。
- 指标卡用语义图标底色区分状态，数值在桌面为 `28px`、手机为 `24px`；正文保留既有字号下限。首页事件卡使用轻微风险底色和完整细边框，信号区单独分组。
...
- KPI 保持两列，数值与说明允许自然换行；Command strip 的严重度与标题并排、四个信号以 `2 × 2` 排列，主动作独占下一行。默认内容下，`320–414px` 宽、`844px` 高的首屏应能看到事件主动作与至少首个完整 KPI；长文本和放大状态优先完整展示，不强行压缩高度。
...
- 2026-09-16 动效补充：页头归位 240ms，搜索/通知/抽屉展开 180–240ms，控件反馈 140–300ms，登录区分组归位 360–420ms。文字在动画中间帧保持完整对比度；仅无文字遮罩允许透明度淡入。关闭弹层立即恢复焦点，页面切换不通过重挂载表单实现动画，数据轮询不重播入场。减少动态效果偏好在运行中开启时取消页面动画，直接显示完整终态。实施与验证见 [frontend-motion.md](docs/design/frontend-motion.md)。
