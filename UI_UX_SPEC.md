...
`/login` 是独立公开入口，不属于原有 22 个业务页面。桌面采用生成的海上风场图片与操作区双栏，手机优先展示登录操作；沿用雾灰/青绿色 token，支持深浅主题。生产模式连接现有 ChatGPT 托管身份登录，保留安全的业务返回路径；演示模式明确标识演示入口。不得通过公开登录页授予生产能力，不引入虚假的账号密码校验。交互必须覆盖断网提示、跳转等待、重复点击和访问帮助。
